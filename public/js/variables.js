...
var r = new Request
r.querySum('/costs/variables/sum', 'head-costs-variables', 'tbody-costs-variables')
r.queryCosts('/costs/variables', 'costs-variables-rows')


function del(id) {
    let conf = confirm("Tem certeza de que deseja exlcuir respectivo custo?!")
    if (conf) {
        fetch('/costs/delete?type=variables&id=' + id).then(del => {
            location.reload();
        })
    }
}